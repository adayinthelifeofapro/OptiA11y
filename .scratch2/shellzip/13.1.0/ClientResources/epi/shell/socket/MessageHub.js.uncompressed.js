define("epi/shell/socket/MessageHub", [
    ...
    "./WebSocket"
], function (declare, lang, Url, Evented, on, topic, when, _ContextMixin, postMessage, WebSocket) {
    return declare([_ContextMixin], {
        ...
        statusTopic: {
            ...
            // globalnotificationmessage: [readonly] String
            //      Broadcase when a socket message is received from the server
            globalnotificationmessage: "/global/notification/message"
        },
        ...
        constructor: function (socketPathResolver) {
            ...
            this._globalNotificationBroadcaster.postMessage("GlobalNotificationBroadcaster initiliazed!");
        },
        start: function () {
            // summary:
            ...
            this._eventListeners.push(on(socket, "error", lang.hitch(this, "_onError")), on(socket, "open", lang.hitch(this, "_onOpen")), on(socket, "close", lang.hitch(this, "_onClose")), on(socket, "message", lang.hitch(this, "_onMessage")));
            this._socket = socket;
        },
        stop: function () {
            // summary:
        ...
        _publish: function (topicName, data) {
            // summary:
            //      Executes a publish on the message hub
            // tags: protected
            this._topicHub.emit(topicName, data);
            if (data && topicName) {
                if (topicName === "/episerver/cms/notification") {
                    this._globalNotificationBroadcaster.postMessage(data);
                }
                if (topicName === "/episerver/cms/content/indexed") {
                    when(this.getCurrentContext()).then(function (context) {
                        if (context.id === data.contentLink) {
                            var contentSavedMessage = Object.assign(data, data.additionalData);
                            if (Object.prototype.hasOwnProperty.call(contentSavedMessage, "additionalData")) {
                                delete contentSavedMessage.additionalData;
                            }
                            postMessage.publish("contentSaved", contentSavedMessage);
                        }
                        else {
                            topic.publish("content-index", Object.assign(data, {
                                currentContentLink: context.id
                            }));
                        }
                    });
                }
        ...
        _onClose: function (e) {
            ...
            // tags: private
            var nextAttempt, publish = function (topic) {
                this._publish(topic, {
