...
// Dojo
array, declare, Deferred, lang, domStyle, 
// Dijit
...
postMessage) {
    return declare([_WidgetBase, _Container, _TemplatedMixin], {
        ...
        updateOverlayItemBoundaries: function (properties) {
            if (!properties) {
                return;
            }
            this.getChildren().forEach(function (item) {
                var property = properties.find(function (prop) {
                    return item.uniqueName && prop.uniqueName.toLowerCase() === item.uniqueName.toLowerCase();
                });
                if (property) {
                    item.updatePosition({
                        h: property.offsetHeight,
                        l: property.offsetLeft,
                        t: property.offsetTop,
                        w: property.offsetWidth,
                        x: property.offsetLeft,
                        y: property.offsetTop
                    });
                }
            });
        },
        _updateOverlayItems: function (targetResized) {
            ...
            //      protected
            //
            if (!this.get("enabled") || this.isUpdating || !(this.target.isInspectable() || this.target.hasSiteProperties())) {
                return;
            }
            var maxIterations = 4;
