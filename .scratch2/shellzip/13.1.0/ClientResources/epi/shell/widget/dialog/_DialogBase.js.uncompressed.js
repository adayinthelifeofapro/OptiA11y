...
define("epi/shell/widget/dialog/_DialogBase", [
...
], function (dojo, aspect, declare, array, domClass, domStyle, query, template, when, a11y, focus, Dialog, registry, DialogUnderlay, _DialogMixin, _FocusableMixin, sharedResources) {
    var module = declare([Dialog, _DialogMixin, _FocusableMixin], {
        ...
        // elementToOverlaySelector: [protected] String
        //      Display the dialog overlay on top of a specific DOM node
        elementToOverlaySelector: null,
        // suppressOverlayAdjustments: [protected] Boolean
        ...
        postMixInProperties: function () {
            ...
            this.resources = sharedResources.action;
        },
        ...
        show: function () {
            ...
            if (!this.suppressOverlayAdjustments && this.elementToOverlaySelector) {
                this.elementToOverlay = document.querySelector(this.elementToOverlaySelector);
                if (this.elementToOverlay) {
                    var widget = registry.byNode(this.elementToOverlay);
                    if (widget) {
                        this.own(aspect.after(widget, "layout", this._resizeOverlay.bind(this)));
                    }
                    this._resizeOverlay();
                }
            }
        ...
        _resizeOverlay: function () {
            ...
            var bounds = this.elementToOverlay.getBoundingClientRect();
            underlay.domNode.style.top = bounds.top + "px";
            underlay.domNode.style.left = bounds.left + "px";
            underlay.domNode.firstChild.style.height = bounds.height + "px";
            underlay.domNode.firstChild.style.width = bounds.width + "px";
            return bounds;
        },
        resize: function () {
            // summary:
        ...
        _getFocusItems: function () {
            ...
            // tags:
            //		protected
            var node = this.focusActionsOnLoad ? this.actionContainerNode : this.contentContainerNode, container = a11y._getTabNavigable(node);
