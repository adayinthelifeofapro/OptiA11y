define("epi/shell/widget/layout/_ComponentSplitter", [
...
], function (declare, event, lang, win, domGeometry, domStyle, keys, on, touch, _WidgetBase, _TemplatedMixin, _CssStateMixin) {
    return declare([_WidgetBase, _TemplatedMixin, _CssStateMixin], {
        ...
        _startDrag: function (e) {
            ...
            var splitterStyle = this.domNode.style, pageStart = e.pageY, node = this.child.domNode, computedStyle = domStyle.getComputedStyle(node), marginExtents = domGeometry.getMarginExtents(node, computedStyle).h, childStart = domGeometry.getMarginBox(node, computedStyle).h, maxFunc = this.container._getMaxSizeChild
                ? lang.hitch(this.container, "_getMaxSizeChild")
                : lang.hitch(this, function () {
                    return this.child.maxHeight || Infinity;
                }), min = this.child.minHeight || 0, splitterStart = parseInt(splitterStyle.top, 10), layoutFunc = lang.hitch(this.container, "_layoutChildren", this.child), doc = win.doc;
            this._handlers = this._handlers.concat([
                on(doc, touch.move, this._drag = function (e, complete) {
                    var delta = e.pageY - pageStart, childSize = delta + childStart, boundChildSize = Math.max(Math.min(childSize, maxFunc()), min + marginExtents);
