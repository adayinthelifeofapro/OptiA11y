define("epi/shell/widget/ComponentSelector", [
...
], function (declare, Deferred, keys, array, lang, domGeometry, OnDemandGrid, Selection, Keyboard, DijitRegistry, _Contained, _Widget, _TemplatedMixin, _WidgetsInTemplateMixin, MenuItem, ContentPane, BorderContainer, Button, TextBox, MemoryStore, routes, dependency, viewSettings, res, sharedResources) {
    var ComponentSelector = declare([_Widget, _TemplatedMixin, _WidgetsInTemplateMixin, _Contained], {
        ...
        show: function () {
            ...
            //    public
            var args = arguments;
            this.componentDefinitionStore.query().then(function (data) {
                var filteredData = array.filter(data, function (item) {
                    return this._inCategoryFilter(item.categories);
                }, this);
                this._populateCategoryMenu(filteredData);
                this.grid.set("store", new MemoryStore({ data: filteredData }));
                this.inherited(args);
                this.resize();
            }.bind(this));
        },
