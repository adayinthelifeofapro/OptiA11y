define("epi/shell/widget/ComponentController", [
...
], function (array, declare, Deferred, lang, aspect, has, topic, all, when, JsonRestStore, MemoryStore, _WidgetBase, dependency, routes, logger, viewSettings, WidgetFactory, ApplicationSettings) {
    return declare([_WidgetBase], {
        ...
        postMixInProperties: function () {
            ...
            // attach callback
            this.own(aspect.after(this.widgetFactory, "onWidgetCreated", lang.hitch(this, this.onComponentCreated), true));
        },
        ...
        addComponent: function (/* dijit/_Widget */ container, /* object */ component, /* Boolean */ createComponent, /* Function? */ callback) {
            ...
            //      public
            //
            if (this._isAddingSystemComponent(component)) {
                return;
            }
            var parentId = this.getComponentDefinition(container.id).id;
            ...
            var self = this;
            when(this._componentsStore.put(newItem), function () {
        ...
        loadComponents: function (/* DomNode|DomNode[] */ nodes) {
            ...
            when(this.getComponentsForView(viewSettings.viewName), function (data) {
                if ( false ) {
        ...
        createComponents: function (/* Object[] */ data, /* DomNode|DomNode[] */ nodes) {
            ...
            //      public
            //
            var factory = this.widgetFactory, items = data, componentsDefinition, node;
        ...
        _isAddingSystemComponent: function (component) {
            // summary:
            //      Check if we are adding a fake system component.
            //      If component already exists we just need to toggle its view settings instead of adding it for real.
            //
            // component:
            //      Added component
            //
            ...
            //      private
            //
            if ([ApplicationSettings.mainNavigationComponentKey, ApplicationSettings.assetsComponentKey].indexOf(component.settings.key) === -1) {
                return false;
            }
            this._componentsStore.query({ viewName: viewSettings.viewName }).then(function (components) {
                var existingComponent = components.find(function (x) {
                    return x.settings.headingLocalizationKey === component.settings.key;
                });
                if (!existingComponent) {
                    return;
                }
                existingComponent.settings.isHidden = false;
                this._componentsStore.executeMethod("save", "", [existingComponent]).then(function () {
