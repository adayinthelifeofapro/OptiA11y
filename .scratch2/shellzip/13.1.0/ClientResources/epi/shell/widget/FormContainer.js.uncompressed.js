...
// dojo
declare, array, lang, domGeom, aspect, has, when, 
// dijit
_LayoutWidget, _TemplatedMixin, _WidgetsInTemplateMixin, Form, 
// epi
epi, logger, WidgetFactory, MetadataTransformer) {
    return declare([_LayoutWidget, _TemplatedMixin, _WidgetsInTemplateMixin], {
        ...
        onWrapperCreated: function (wrapper) {
            // summary:
            //     Raised when a field wrapper is created.
            // wrapper: object
            //     The wrapper
            // tags:
            //     callback
        ...
        _setupUI: function () {
            ...
            when(this.metadata, lang.hitch(this, function (item) {
                if ( false ) {
                ...
                require([layoutClass], lang.hitch(this, function (layoutCtor) {
                    var parentWidgets = [], groupWidgets = [];
                    this.containerLayout = new layoutCtor();
                    parentWidgets.push(this.containerLayout);
                    this.connect(widgetFactory, "onWrapperCreated", function (wrapper) {
                    ...
                    when(widgetFactory.createWidgets(this.containerLayout, componentDefinitions, false)).then(function () {
                        if (this._destroyed) {
                            if ( false ) {
                                timeLog.end();
                            }
        ...
        assignBlockGuids: function (value) {
            // summary:
            //     Deserializing blocks requires contentTypeGuid to be provided
            // value: object
            //     form value
            // tags:
            //     private
            if (!value || !this.metadata || !this.metadata.properties) {
                return;
            }
            this.metadata.properties.forEach(function (property) {
                if (property.uiType !== null) {
                    return;
                }
                if (property.additionalValues === null || !property.additionalValues.contentTypeGuid) {
                    return;
                }
                var valueProperty = Object.keys(value).find(function (x) {
                    return x.toLowerCase() == property.name.toLowerCase();
                });
                if (valueProperty) {
                    value[valueProperty].contentTypeGuid = property.additionalValues.contentTypeGuid;
                }
            });
        }
    });
