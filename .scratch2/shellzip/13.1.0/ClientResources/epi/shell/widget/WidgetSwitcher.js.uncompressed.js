define("epi/shell/widget/WidgetSwitcher", [
...
], function (array, declare, lang, Deferred, topic, when, StickyViewSelector, _ContextMixin, AnimatedCardContainer, TypeDescriptorManager) {
    return declare([AnimatedCardContainer, _ContextMixin], {
        ...
        postCreate: function () {
            ...
            this.own(topic.subscribe("/epi/shell/action/changeview", lang.hitch(this, "viewComponentChangeRequested")), topic.subscribe("/epi/shell/action/changeview/back", lang.hitch(this, "_viewComponentBackRequested")), topic.subscribe("/epi/shell/action/changeview/updatestate", lang.hitch(this, "_changeViewState")));
        },
        hasTemplateWithPreview: function (context) {
            return !!context && context.hasTemplate && !!context.previewUrl;
        },
        contextChanged: function (/*Object*/ context, data) {
            ...
            if (view) {
                ...
                var suggestedView = TypeDescriptorManager.getValue(context.dataType, "defaultView"), availableViews = TypeDescriptorManager.getAvailableViews(context.dataType), requestedViewName = data ? data.viewName : null;
                // try to load last selected view
                when(this._stickyViewSelector.get(this.hasTemplateWithPreview(context), context.dataType))
                    ...
                    var availableViewsKeys = (availableViews || []).map(function (availableView) {
                        return availableView.key;
                    });
                    if (availableViewsKeys.indexOf(suggestedView) === -1) {
                        if (availableViewsKeys.indexOf("formedit") !== -1) {
                            suggestedView = "formedit";
                        }
                    }
                    var viewToLoad = this._getViewByKey(requestedViewName, availableViews) || this._getViewByKey(suggestedView, availableViews);
                    if (!viewToLoad) {
        ...
        _getDefaultViewName: function (dataType, hasTemplate) {
            // summary:
            //      Get the defaultView for the given dataType
            //      In the case the defaultView isn't supported for data types without a view
            //      template (hasTemplate is false), another available view will be returned.
            // tags:
            //      private
            var viewsArr = [
                "view",
            ...
            var defaultViewName = TypeDescriptorManager.getValue(dataType, "defaultView");
            if ((hasTemplate === false) && viewsArr.indexOf(defaultViewName) > -1) {
                var availableViews = TypeDescriptorManager.getAvailableViews(dataType, !!hasTemplate === false);
                return availableViews ? availableViews[0].key : null;
            }
        ...
        viewComponentChangeRequested: function (/*String*/ type, /*Object*/ args, /*Object*/ data, /*Boolean?*/ saveView) {
            ...
            when(this.getCurrentContext(), lang.hitch(this, function (currentContext) {
                ...
                var hasTemplate = this.hasTemplateWithPreview(currentContext);
                var availableViews = TypeDescriptorManager.getAvailableViews(dataType);
                var defaultViewName;
                var stickyView;
                // If no view or widget type is supplied, then use the default view for the content type
                if (!type && dataType) {
                    defaultViewName = _this._getDefaultViewName(dataType, hasTemplate);
            ...
            var func = lang.hitch(this, function () {
                when(this._testComponentInstanceOf(current, type), lang.hitch(this, function (result) {
                    if (saveView) {
                        ...
                        this._stickyViewSelector.save(this.hasTemplateWithPreview(context), context.dataType, data.viewName);
                    }
                    if (result) {
                        this._updateHistory(type, data, context);
                        this._updateView(current, context, data);
                        this._updateDataProperties(context, data);
                        this._onViewChanged(type, args, data, context);
                    }
                    else {
        ...
        _updateDataProperties: function (context, data) {
            data.hideView = !this.hasTemplateWithPreview(context);
            data.dataType = context.dataType;
            var viewConfiguration = null;
            if (data && data.availableViews && data.availableViews.find) {
                viewConfiguration = data.availableViews.find(function (x) {
                    return x.key === data.viewName;
                });
            }
            ...
            when(this._getChildByType(type), lang.hitch(this, function (component) {
                ...
                if (component) {
                    ...
                    require([type], lang.hitch(this, function (componentClass) {
                        var mixedArgs = lang.mixin(lang.mixin({}, this.componentConstructorArgs, { componentTypeName: type }), args);
        ...
        _onViewChanged: function (type, args, data, context) {
            topic.publish("/epi/shell/action/viewchanged", type, args, data, context);
        },
        _updateHistory: function (/*string*/ type, data, context) {
            ...
            // tags:
            //      private
            var history = this._viewHistory, length = history.length, item = this.createStoreItem(type, data, context);
        ...
        createStoreItem: function (/*string*/ type, data, context) {
            ...
            if (data) {
                var availableViews = data.availableViews || (context && TypeDescriptorManager.getAvailableViews(context.dataType));
        ...
        _getViewName: function (availableViews, context) {
            ...
            var selectedView = this._getViewByKey(this._getDefaultViewName(context.dataType, this.hasTemplateWithPreview(context)), availableViews);
            if (!selectedView) {
                return null;
            }
