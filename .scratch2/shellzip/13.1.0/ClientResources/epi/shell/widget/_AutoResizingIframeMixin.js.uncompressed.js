...
// Dojo
declare, Deferred, domAttr, domStyle, has, postMessage, _ScrollbarMeasurementMixin) {
    return declare([_ScrollbarMeasurementMixin], {
        ...
        onElementResize: function (properties) {
            // summary:
            //      Called when the iframe receives the resize message from the loaded document
            // tags:
            //      callback
        },
        postCreate: function () {
            var self = this, topicName = this.name + "/site/resize";
            this.inherited(arguments);
            this.own(postMessage.subscribe(topicName, function (message) {
                self.onElementResize(message.properties);
                self.contentChange(message);
            }));
        },
        checkSize: function () {
            if (this.domNode.checkVisibility()) {
                postMessage.publish("/site/checksize", {});
            }
        },
        onLoad: function () {
        ...
        resize: function (size, force, documentSize) {
            ...
            // size: Object
            //    New size {w, h}
            var self = this, def = this._resize(size, force, documentSize);
        ...
        _resize: function (size, force, documentSize) {
            var previousSize = this._contentBox || { h: 0, w: 0 }, self = this, internalDef = new Deferred(), externalDef = new Deferred(), viewportSize = {
                h: size.h,
                w: size.w
            };
        ...
        _setToDocumentSize: function (viewportSize) {
            ...
            var docSize = {};
            // can we access the contentDocument or does cross domain prevent us?
