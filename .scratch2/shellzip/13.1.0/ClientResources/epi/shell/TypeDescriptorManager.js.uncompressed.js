...
// dojo
array, declare, lang, all, when, 
// epi
dependency, typeResources) {
    return {
        // summary:
        ...
        registerTypeSettings: function (result) {
            var uiDescriptors = result.uiDescriptors;
            var types = this._uiDescriptors = {};
            this._contentTypeDescriptors = result.contentTypeDescriptors;
            function getOrCreateDeclaration(typeKey) {
                var type = types[typeKey], uiDescriptor = type.descriptor, baseTypes = uiDescriptor.baseTypes || [];
                if (type.ctor) {
                    return type.ctor;
                }
                if (type.visited) {
        ...
        registerTypeSetting: function (name, setting) {
            ...
            this.registerTypeSettings({ uiDescriptors: [setting] });
        },
        ...
        getTypeNameFromTypeId: function (contentTypeId) {
            // summary:
            //    Gets the content type name based on contentTypeId.
            //
            // contentTypeId: number | string
            //    Id of the content type
            var typeId = parseInt(contentTypeId);
            var contentTypeDescriptor = this._contentTypeDescriptors && this._contentTypeDescriptors.find(function (descriptor) {
                return descriptor.id === typeId || descriptor.name === contentTypeId;
            });
            if (contentTypeDescriptor) {
                return contentTypeDescriptor.name;
            }
            return null;
        },
        getTypeName: function (type) {
            var isValidInteger = /^\d+$/.test(type);
            var isBaseType = this.getBaseTypeNames().indexOf(type) !== -1;
            if (isValidInteger || isBaseType) {
                var typeId = parseInt(type, 10);
                var contentTypeDescriptor = this._contentTypeDescriptors && this._contentTypeDescriptors.find(function (descriptor) {
                    return descriptor.id === typeId || descriptor.name === type;
                });
                if (contentTypeDescriptor) {
                    return contentTypeDescriptor.localizedName;
                }
            }
            var typeDescriptor = this._uiDescriptors[type];
            if (!typeDescriptor) {
            ...
            return typeDescriptor.descriptor.typeName || typeDescriptor.instance.getResource("name");
        },
        getAndConcatenateValues: function (type, settingName) {
            ...
            // settingName: string
            //    The settings that you want to fetch.
            var values = [], typeDescriptor = this._uiDescriptors[type];
        ...
        getInheritanceChain: function (type) {
            ...
            var value = [], typeDescriptor = this._uiDescriptors[type];
            if (!typeDescriptor) {
                return value;
            }
        ...
        getDerivedTypes: function (type) {
            // summary:
            //    Returns an array of types which are based on the given type
            //
            // type: string
            //    The type that you want to fetch the derived types for
            var result = [];
            var visited = {};
            var collectDerived = function (currentType) {
                if (visited[currentType]) {
                    return;
                }
                visited[currentType] = true;
                result.push(currentType);
                Object.keys(this._uiDescriptors).forEach(function (key) {
                    var instance = this._uiDescriptors[key];
                    if (instance.descriptor.baseTypes &&
                        instance.descriptor.baseTypes.indexOf(currentType) !== -1) {
                        collectDerived.call(this, key);
                    }
                }, this);
            };
            collectDerived.call(this, type);
            return result;
        },
        isBaseTypeIdentifier: function (childType, parentType) {
            ...
            // parentType: String
            //      the parent type
            // no need to check inheritance if types are exact match
        ...
        _prepareDescriptor: function (uiDescriptor) {
            var resource, languageKey = uiDescriptor.languageKey;
            ...
            lang.mixin(uiDescriptor, {
                ...
                getResource: function (key, getPrimary) {
                    var resources = uiDescriptor.resource, resource;
                    if (resources) {
        ...
        getAvailableViews: function (dataType, withoutView) {
            // summary:
            //      Returns a sorted list of available views for the given dataType
            // dataType: [String]
            //      The type that you want to get the available views for.
            // withoutView: [Boolean]
            //      Indicate whether the current content is with or without a view.
            //      In the case withoutView is true, three following views will be removed
            //      from the available view list: "view" (Preview Mode),
            //      "onpageedit" (On-page Editing) and "sidebysidecompare" (On-page comparison)
            var availableViews = this.getAndConcatenateValues(dataType, "availableViews");
            if (!availableViews || availableViews.length === 0) {
                return [];
            }
            var disabledViews = this.getValue(dataType, "disabledViews");
            ...
            if (withoutView) {
                filteredViews = array.filter(filteredViews, function (view) {
                    return view.key !== "view" && view.key !== "onpageedit" && view.key !== "sidebysidecompare";
                });
            }
        ...
        getBaseTypeNames: function () {
            return ["Page", "Block", "Media", "Video", "Image", "Folder"];
        }
    };
});
