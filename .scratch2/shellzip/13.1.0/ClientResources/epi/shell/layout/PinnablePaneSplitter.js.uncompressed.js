define("epi/shell/layout/PinnablePaneSplitter", [
    ...
    "dojo/on",
    "dojo/touch"
], function (dojo, domClass, domConstruct, domGeometry, domStyle, on, touch) {
    return function (splitter) {
        ...
        return dojo.mixin(splitter, {
            _startDrag: function (e) {
                ...
                var factor = this._factor, isHorizontal = this.horizontal, axis = isHorizontal ? "pageY" : "pageX", pageStart = e[axis], splitterStyle = this.domNode.style, dim = isHorizontal ? "h" : "w", childStart = domGeometry.getMarginBox(this.child.containerNode)[dim], max = this._computeMaxSize(), min = this.child.minSize || 20, region = this.region, splitterAttr = region === "top" || region === "bottom" ? "top" : "left", // style attribute of splitter to adjust
                splitterStart = parseInt(splitterStyle[splitterAttr], 10), resizeFunc = dojo.hitch(this.child, "resize"), layoutFunc = dojo.hitch(this.container, "_layoutChildren", this.child.id), de = dojo.doc, pinned = this.child.pinned;
