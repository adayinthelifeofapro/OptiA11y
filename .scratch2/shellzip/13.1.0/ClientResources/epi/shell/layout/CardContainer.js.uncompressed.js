define("epi/shell/layout/CardContainer", [
    ...
    "dgrid/util/has-css3"
], function (declare, lang, fx, aspect, has, on, domClass, domStyle, domGeometry, Deferred, when, _TemplatedMixin, StackContainer, hasCss3) {
    return declare([StackContainer, _TemplatedMixin], {
        ...
        _doTransition: function (page, topPage, opening, currentOffset) {
            ...
            if (page !== topPage) {
                var marginBox = domGeometry.getMarginBox(page.domNode), animationNode = this._getAnimationNode(page, topPage);
                var property = this.transitionDirection === "vertical" ? "top" : "left", size = this.transitionDirection === "vertical" ? marginBox.h : marginBox.w, from = opening ? -(size - currentOffset) + "px" : "0px", to = opening ? "0px" : -size + "px", restore = !opening;
