define("epi/shell/layout/PositioningUtility", [
    ...
    "dojo/dom-style",
    "dojo/window"
], function (declare, lang, dom, domGeom, domStyle, win) {
    return declare(null, {
        // summary:
        ...
        place: function (/*Object*/ elementToPosition, /*Object*/ settings) {
            ...
            var node = this._getNode(elementToPosition), pos = this.findPosition(node, settings);
            domStyle(node, {
                position: "absolute",
                top: pos.y + "px",
                left: pos.x + "px"
        ...
        findPosition: function (/*Object*/ elementToPosition, /*Object*/ settings) {
            ...
            //      An object containing settings:
            //          - snapTo: The node|id|position elementToPosition should dock to.
            var blockViewport = settings.blockViewport || settings.frame || this._getFramePosition(settings.snapTo, elementToPosition), blockPos = this._getVisiblePosition(this._getPosition(settings.snapTo), blockViewport), dialogViewport = settings.dialogViewport || win.getBox(), dialogPos = this._getPosition(elementToPosition);
        ...
        _getFramePosition: function (blockNode, dialogNode) {
            var iframe = blockNode.ownerDocument.defaultView.frameElement, position;
            function iterateIframes(iframe, position) {
                var doc = iframe.ownerDocument, pos = domGeom.position(iframe);
                if (!position) {
                    position = pos;
                }
                else {
        ...
        _executeTransformers: function (pos, block, viewport, transformIndex) {
            ...
            var position = null, transformers = isNaN(transformIndex) ? this._transformers : [this._transformers[transformIndex]];
