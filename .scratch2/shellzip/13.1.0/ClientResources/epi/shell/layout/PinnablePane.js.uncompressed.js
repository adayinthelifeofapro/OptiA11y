...
define("epi/shell/layout/PinnablePane", [
...
], function (array, declare, event, lang, win, domClass, domGeometry, domStyle, topic, when, template, _Contained, _Container, _TemplatedMixin, ContentPane, dependency, resources, PinnablePaneSplitter, _ActionConsumerWidget, _ActionProvider, ToolbarSet) {
    return declare([ContentPane, _TemplatedMixin, _Contained, _Container, _ActionConsumerWidget], {
        ...
        postCreate: function () {
            ...
            // tags:
            //		protected
            var directionClass = "", ltr = this.isLeftToRight(), startDrag = function () {
                if (this.visible) {
                    this._dragging = true;
                }
            }, endDrag = function () {
        ...
        resize: function (changeSize) {
            ...
            // tags:
            //		public
            var pos = this.region, hasWidth = this.visible && this.pinned, dockSize = domGeometry.getMarginBox(this.domNode), currentSize = domGeometry.getMarginBox(this.containerNode), containerBox = {}, pinnableBox = { t: dockSize.t }, isLeft = pos === "left" || pos === "leading", isRight = pos === "right" || pos === "trailing";
            ...
            if (this._splitterWidget) {
                var splitterSize = domGeometry.getMarginBox(this._splitterWidget.domNode), splitterBox = { t: dockSize.t };
                if (isLeft) {
        ...
        getActions: function () {
            ...
            // tags:
            //		protected
            var actions = this.inherited(arguments), isLeft = /left|leading/.test(this.region), pin = {
                name: "pin",
        ...
        _computeMaxSize: function () {
            ...
            // tags:
            //		private
            var childSize = domGeometry.getMarginBox(this.domNode).w, center = array.filter(this._container.getChildren(), function (child) {
                return child.region === "center";
            })[0], spaceAvailable = domGeometry.getMarginBox(center.domNode).w; // can expand until center is crushed to 0
