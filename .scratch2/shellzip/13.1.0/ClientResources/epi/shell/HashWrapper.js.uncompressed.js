define("epi/shell/HashWrapper", [
    ...
    return declare([StateFul], {
        ...
        _normalizeURIString: function (uriString) {
            try {
                var uri = decodeURIComponent(uriString);
                if (encodeURI(uri) === uriString || encodeURIComponent(uri) === uriString) {
                    return uriString;
                }
                return uri;
            }
        ...
        getHash: function () {
            ...
            var uriString = this._normalizeURIString(currentHash);
            return this._queryToObject(uriString);
        },
        consume: function (/*string*/ fragmentName) {
            // summary:
            //      Consum the hash value of the fragmentName
            //
            // fragmentName: String
            //      Name of fragment in current hash.
            //
            // tags:
            //      internal
            var currentHash = this.getHash();
            if (currentHash && Object.prototype.hasOwnProperty.call(currentHash, fragmentName)) {
                var fragmentValue = currentHash[fragmentName];
                delete currentHash[fragmentName];
                this._requestContext = false; // prevent onHashChanged is raised.
                this.setHash(currentHash);
                return fragmentValue;
            }
            return undefined;
        },
        _queryToObject: function (hashString) {
            var dec = decodeURIComponent, params = Array.from(hashString.split(/&(?=\w+=)/)), name, val;
        ...
        setHash: function (fragment) {
            ...
            // tags:
            //      Public
            this._hash(this._normalizeURIString(decodeURIComponent(io.objectToQuery(fragment))));
        },
        hashEmpty: function () {
            // summary:
        ...
        _hashAreEqual: function (firstHash, secondHash) {
            ...
            return epi.areEqual(firstHash, secondHash);
        },
        ...
        extractHash: function (ctx, callerData) {
            ...
            return decodeURIComponent(io.objectToQuery(fragment));
        },
        _onContextChange: function (ctx, callerData) {
            ...
            if (!this._hashAreEqual(storedHash, newHash)) {
                if (!callerData || callerData.sender !== this) {
                    ...
                    this._hash(this._normalizeURIString(this.extractHash(ctx, callerData)));
                }
        ...
        _onHashChanged: function (newHash) {
            ...
            if (this._requestContext) {
                ...
                var uri = this._normalizeURIString(newHash);
                var hashObj = this._queryToObject(uri);
