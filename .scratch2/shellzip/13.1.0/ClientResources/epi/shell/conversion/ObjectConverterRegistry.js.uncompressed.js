define("epi/shell/conversion/ObjectConverterRegistry", [
    "dojo/_base/array",
    "dojo/_base/lang",
    "epi-cms/conversion/ContentReferenceConverter"
], function (array, lang, ContentReferenceConverter) {
    return {
        // summary:
    ...
    */
        ...
        getDynamicReferenceConverter: function (sourceDataTypes, targetDataType) {
            // for database content types we can't register converters when starting modules
            // we need to check if source type and target types are database content types that can be converted
            function getSourceRegex() {
                return new RegExp("(?<name>[0-9]+)(?:-[\\w]+)?\\.fragment", "g");
            }
            function getTargetRegex() {
                return new RegExp("(?<name>[0-9]+)(\\.reference)?", "g");
            }
            if (!lang.isArray(sourceDataTypes)) {
                sourceDataTypes = [sourceDataTypes];
            }
            function getContentTypeId(regex, str) {
                var result = Array.from(str.matchAll(regex));
                for (var i = 0; i < result.length; i++) {
                    return result[i].groups.name;
                }
                return str;
            }
            for (var i = 0; i < sourceDataTypes.length; i++) {
                var sourceDataType = sourceDataTypes[i];
                if (!getSourceRegex().test(sourceDataType)) {
                    continue;
                }
                if (!getTargetRegex().test(targetDataType)) {
                    continue;
                }
                // check if references match
                if (getContentTypeId(getSourceRegex(), sourceDataType) !== getContentTypeId(getTargetRegex(), targetDataType)) {
                    continue;
                }
                var converterResult = {};
                converterResult[targetDataType] = new ContentReferenceConverter();
        ...
        getConverter: function (/*String*/ sourceDataType, /*String*/ targetDataType) {
            ...
            var converter;
            // add dynamic database converter
            var dynamicConverter = this.getDynamicReferenceConverter(sourceDataType, targetDataType);
            if (dynamicConverter) {
                if (!converters) {
                    converters = [];
                }
                converters.push(dynamicConverter);
            }
