define("epi/shell/ShellModule", [
...
], function (patches, declare, lang, aspect, topic, when, dndManager, epi, _Module, routes, Url, applicationSettings, StoreRegistry, ApplicationWidget, Profile, PositioningUtility, ContextService, ContextHistory, HashWrapper, ViewContextHistory, ComponentController, GlobalCommandRegistry, Avatar, MetadataManager, WidgetFactory, MessageService, TypeDescriptorManager, viewSettings, StickyViewSelector, hub) {
    return declare([_Module], {
        ...
        _initializeStores: function (registry) {
            var metadataStoreUrl = routes.getRestPath({ moduleArea: "shell", storeName: "metadata" }), contextStoreUrl = routes.getRestPath({ moduleArea: "shell", storeName: "context" }), searchProvidersUrl = routes.getRestPath({ moduleArea: "shell", storeName: "searchproviders" }), searchResultsUrl = routes.getRestPath({ moduleArea: "shell", storeName: "searchresults" }), componentCategoryUrl = routes.getRestPath({ moduleArea: "shell", storeName: "componentcategory" }), componentDefinitionUrl = routes.getRestPath({ moduleArea: "shell", storeName: "componentdefinition" }), componentUrl = routes.getRestPath({ moduleArea: "shell", storeName: "component" }), componentSortOrderUrl = routes.getRestPath({ moduleArea: "shell", storeName: "componentsortorder" }), licenseinformationUrl = routes.getRestPath({ moduleArea: "shell", storeName: "licenseinformation" }), uiDescriptorUrl = routes.getRestPath({ moduleArea: "shell", storeName: "uidescriptor" });
        ...
        _requestInitialContext: function (hashWrapper, viewContextHistory) {
            ...
            var _tryGetInitialUri = function (context) {
                if (!context || !context.hosts) {
                    return false;
                }
                var currentHost = window.location.host.toLowerCase();
                for (var i = 0; i < context.hosts.length; i++) {
                    var host = context.hosts[i];
                    if (!host) {
                        continue;
                    }
                    var url = new URL(host);
                    if (url.host.toLowerCase() === currentHost) {
                        return true;
                    }
                }
                return false;
            };
            var _isContextPartOfRoot = function (context) {
                if (!context || !context.hosts) {
                    return false;
                }
                // This should cover the case where the context is being loaded from a page that is not part of any site, it's part of a root.
                return context.hosts.length === 0;
            };
            ...
            when(viewContextHistory.getLastContextForView(), function (context) {
                var initialUri = null;
                if (_isContextPartOfRoot(context) || _tryGetInitialUri(context)) {
