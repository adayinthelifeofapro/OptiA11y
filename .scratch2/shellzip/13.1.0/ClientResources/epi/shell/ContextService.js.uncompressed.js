...
// Dojo
dojo, array, declare, lang, Deferred, all, topic, entities, Destroyable, 
// EPi
dependency, dialogService, UriParser, 
// Resources
res, sharedResources) {
    return declare([Destroyable], {
        ...
        _initialize: function () {
            this.own(topic.subscribe("/epi/shell/context/request", lang.hitch(this, this._handleContextChangeRequest)), topic.subscribe("/epi/shell/context/updateRequest", lang.hitch(this, this._handleContextUpdateRequest)), topic.subscribe("/epi/shell/context/requestcurrent", lang.hitch(this, this._handleRequestCurrentContext)), topic.subscribe("/epi/shell/context/refreshcurrent", lang.hitch(this, this._handleRefreshCurrentContext)));
        },
        ...
        _handleRefreshCurrentContext: function () {
            topic.publish("/epi/shell/context/request", this.currentContext, {
                sender: this,
        ...
        _contextLoadFailed: function (response, contextParameters, callerData) {
            var defaultError = this.res.errors.couldnotloadcontext.description;
            var errors = [];
            if (response && response.response) {
                try {
                    errors = JSON.parse(response.response.data);
                }
                catch (err) {
                    // If we fail to parse any errors from the response, we just show a generic error message.
            ...
            var errorDescription = this._htmlEscape(errors.join("<br/>"));
            this._dialogService.confirmation({
                ...
                cancelActionText: sharedResources.action.ignore
            }).then(function () {
                // Try again
