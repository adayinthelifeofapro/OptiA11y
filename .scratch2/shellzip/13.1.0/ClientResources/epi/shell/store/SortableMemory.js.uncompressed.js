...
Memory) {
    return declare(Memory, {
        ...
        getSibling: function (object, previous) {
            ...
            // returns:
            //      Object
            var data = this.data, index = this.index, id = this.getIdentity(object);
            if (!(id in index)) {
                throw new Error("Object specified in options.before does not exist");
            }
            var delta = previous ? -1 : 1, pos = index[id] + delta;
        ...
        put: function (object, options) {
            ...
            if (options) {
                ...
                if (options.before || (Object.prototype.hasOwnProperty.call(options, "before") && existingObject)) {
                    // Get the previous index and default the destination to the last item.
                    var previousIndex = this.index[id], destination = data.length;
