...
TemplateColumn) {
    var module = declare(null, {
            ...
            array.forEach(this.subRows, function (columns) {
                array.forEach(columns, function (column) {
                    ...
                    array.forEach(module.shorthands, function (shorthand) {
                        var formatterActive = column[shorthand], formatter = module.formatters[shorthand];
                        if (formatterActive) {
                            if (formatter) {
        ...
        renderRow: function (object, options) {
            ...
            var row = this.inherited(arguments, ["", options]), data = this.field ? object[this.field] : object, formatters = this._resolvedFormatters, i = -1;
            function executeFormatter(value) {
                var formatter = formatters[i], result;
                if (formatter) {
                    row.innerHTML = value;
                    data = value;
                }
        ...
        _findFormatters: function (formatterConfig, formatters) {
            ...
            array.forEach(formatterConfig, function (formatterKey) {
                var type = typeof formatterKey, item, formatter;
                if (type === "string") {
