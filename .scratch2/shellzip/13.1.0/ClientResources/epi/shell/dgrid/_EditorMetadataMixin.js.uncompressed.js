define("epi/shell/dgrid/_EditorMetadataMixin", [
    ...
    "epi/string",
    "./_MetadataMixin"
], function (array, declare, lang, Deferred, all, editor, epiString, _MetadataMixin) {
    return declare([_MetadataMixin], {
        ...
        _metadataPropertiesToColumnsDefinition: function (properties, include) {
            ...
            // tags:
            //		private
            var colDefs = this.inherited(arguments), filteredProps = this._filterProperties(properties, this.metadata.gridEditable), uiWidgets = [];
            ...
            require(uiWidgets, lang.hitch(this, function () {
                for (var i = 0, j = filteredProps.length; i < j; i += 1) {
                    var prop = filteredProps[i], widget = arguments[array.indexOf(uiWidgets, prop.uiType)], colName = epiString.pascalToCamel(prop.name), selections = prop.selections && prop.selections.length > 0 ? { selections: prop.selections } : {};
                    colDefs[colName] = this.editorType(lang.mixin(colDefs[colName], // the column definition settings
                    this.editorSettings, {
                        editor: widget,
                        editorArgs: lang.mixin(colDefs[colName].editorArgs, this.editorArgs, // grid.editorArgs settings
        ...
        startup: function () {
            ...
            var inherited = this.getInherited(arguments), args = arguments;
