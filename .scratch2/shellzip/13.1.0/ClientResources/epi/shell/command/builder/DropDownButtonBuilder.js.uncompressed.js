...
], function (declare, lang, domClass, Menu, MenuSeparator, DropDownButton, MenuBuilder, MenuWithSeparatorBuilder, DestroyableByKey) {
    return declare([MenuWithSeparatorBuilder, DestroyableByKey], {
        // summary:
        ...
        remove: function (command, container) {
            ...
            // tags:
            //      public
            var button = this._button, children = button.dropDown ? button.dropDown.getChildren() : [];
            children.forEach(function (child, index) {
                if (child._command === command) {
                    this.destroyByKey(command);
                    button.removeChild(child);
                    child.destroy();
                    if (child._commandCategory === "menuWithSeparator" && index < children.length - 1) {
                        var separator = children[index + 1];
                        if (separator instanceof MenuSeparator) {
                            button.removeChild(separator);
                            separator.destroy();
                        }
                    }
                    return true;
        ...
        _addToContainer: function (widget, container) {
            ...
            //		protected
            if (!this._button) {
                this._button = new DropDownButton(Object.assign({ dropDown: new Menu() }, this.settings));
                this._button._commandCategory = widget._commandCategory;
            }
            if (!this._button._started) {
                this._addButtonToContainer(container);
                this._button.startup();
            }
            this.inherited(arguments, [widget, this._button.dropDown]);
            this._toggleButton();
            this.ownByKey(widget._command, widget._command.watch("isAvailable", this._toggleButton.bind(this)));
        },
        _toggleButton: function () {
            ...
            // tags:
            //      private
            var children = this._button.dropDown.getChildren(), len = children.length, i = 0, hideButton = true, child;
            for (i; i < len; i++) {
                child = children[i];
                ...
                domClass.toggle(child.domNode, "dijitHidden", !child._command.isAvailable);
                child.set("disabled", !child._command.canExecute);
            }
