...
define("epi/patch/dijit/tree/Tree", [
    ...
    "dijit/registry",
    "dijit/Tree"
], function (array, lang, DeferredList, on, touch, _Container, registry, Tree) {
    lang.mixin(Tree._TreeNode.prototype, {
        setChildItems: function (/* Object[] */ items) {
            ...
            var tree = this.tree, model = tree.model, defs = []; // list of deferreds that need to fire before I am complete
            ...
            this.defer(function () {
                array.forEach(oldChildren, function (node) {
                    if (!node._destroyed && !node.getParent()) {
                        ...
                        // Should wrap under try catch to avoid exception when selection nodes is already destroyed.
                        // That only happens when select sub node and then switch site.
                        // Then we can continue to remove from _itemNodesMap and destroy the node.
                        try {
                            tree.dndController.removeTreeNode(node);
                        }
                        catch (e) {
                            console.warn("Error when removing node from selection: " + e);
                        }
